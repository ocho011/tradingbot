...
from src.services.candle_data_manager import CandleDataManager, SymbolConfig, SystemMetrics
...
class TestEdgeCases:
    ...
    async def test_symbol_config_dataclass(self):
        ...
        config = SymbolConfig(symbol="BTCUSDT", timeframes={TimeFrame.M1, TimeFrame.M15})
        assert config.symbol == "BTCUSDT"
        assert len(config.timeframes) == 2
        assert config.enabled is True
    ...
    async def test_system_metrics_dataclass(self):
        ...
        metrics = SystemMetrics(
            cpu_percent=45.5, memory_percent=60.3, total_candles=1000, active_symbols=5
        )
