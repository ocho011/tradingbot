...
from src.services.risk.daily_loss_monitor import DailyLossLimitError, DailyLossMonitor
...
class TestDailyLossMonitorInitialization:
    ...
    def test_successful_initialization(self):
        ...
        monitor = DailyLossMonitor(
            event_bus=event_bus, daily_loss_limit_pct=6.0, reset_time_utc=time(0, 0), precision=8
        )
...
class TestLossLimitDetection:
    ...
    def test_loss_limit_exactly_at_threshold(self):
        ...
        with patch("asyncio.run") as mock_run, patch("asyncio.get_event_loop") as mock_get_loop:
            mock_loop = Mock()
    ...
    def test_loss_limit_exceeded(self):
        ...
        with patch("asyncio.run"), patch("asyncio.get_event_loop") as mock_get_loop:
            mock_loop = Mock()
            mock_loop.is_running.return_value = False
            mock_get_loop.return_value = mock_loop
...
class TestEventEmission:
    ...
    def test_event_priority(self):
        ...
        with (
            patch("src.services.risk.daily_loss_monitor.Event") as mock_event_class,
            patch("asyncio.run"),
        ):
        ...
        def update_balance_thread(pnl_value):
            try:
                result = monitor.update_balance(
                    Decimal("9500"), Decimal(str(pnl_value)), Decimal("0")
                )
...
class TestEdgeCases:
    ...
    def test_very_small_balance(self):
        ...
        with patch("asyncio.run"):
            result = monitor.update_balance(
                Decimal("0.000000009"), Decimal("-0.000000001"), Decimal("0")
            )
    ...
    def test_very_large_balance(self):
        ...
        with patch("asyncio.run"):
            result = monitor.update_balance(large_balance - loss, -loss, Decimal("0"))
    ...
    def test_mixed_realized_unrealized_pnl(self):
        ...
        result = monitor.update_balance(Decimal("9900"), Decimal("200"), Decimal("-300"))
        assert result["session"].total_pnl == Decimal("-100")

        # Realized loss, unrealized profit
        result = monitor.update_balance(Decimal("10100"), Decimal("-300"), Decimal("400"))
        assert result["session"].total_pnl == Decimal("100")
