...
class TestStrategyConfigManager:
    ...
    def config_manager(self, temp_config_file):
        ...
        return StrategyConfigManager(
            ...
            auto_save=False  # Disable auto-save for most tests
        )
    ...
    def test_save_config_success(self, config_manager, temp_config_file):
        ...
        assert saved_data['strategies'][StrategyType.STRATEGY_A.value]['confidence_threshold'] == 90.0
    ...
    def test_load_config_success(self, config_manager, temp_config_file):
        ...
        assert result is True
        assert new_manager.config.strategies[StrategyType.STRATEGY_A.value].confidence_threshold == 95.0
        ...
        def update_config(strategy_idx):
            try:
                for _ in range(updates_per_thread):
                    ...
                    config_manager.update_strategy_params(
                        strategy_name,
                        {'confidence_threshold': 70.0 + strategy_idx}
                    )
    ...
    def test_config_history_rollback(self, config_manager):
        ...
        # Save initial state
        initial_threshold = config_manager.config.strategies[strategy_name].confidence_threshold
