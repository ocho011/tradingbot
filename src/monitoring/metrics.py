...
def record_signal_generated(strategy: str, symbol: str, direction: str) -> None:
    """
...
def record_order_execution(symbol: str, order_type: str, side: str, execution_time: float) -> None:
    """
...
def record_risk_violation(violation_type: str, symbol: str, severity: str = "medium") -> None:
    """
    ...
    try:
        trading_metrics.risk_violations.labels(
            violation_type=violation_type, symbol=symbol, severity=severity
        ).inc()
        logger.warning(
            f"Recorded risk violation: {violation_type} for {symbol} (severity: {severity})"
        )
    except Exception as e:
        logger.error(f"Failed to record risk violation metric: {e}")


def update_position_pnl(symbol: str, side: str, pnl: float) -> None:
    """
    ...
    """
    try:
        trading_metrics.position_pnl.labels(symbol=symbol, side=side).set(pnl)
        logger.debug(f"Updated position P&L: {symbol} {side} = {pnl:.2f} USDT")
    except Exception as e:
        logger.error(f"Failed to update position P&L metric: {e}")


def record_websocket_connection(exchange: str, stream_type: str, active: bool) -> None:
    """
