...
class TestBasicOperations:
    ...
    def test_get_candles_with_time_range(self, storage):
        ...
        candles = storage.get_candles(
            "BTCUSDT", TimeFrame.M1, start_time=start_time, end_time=end_time
        )
...
class TestLRUEviction:
    ...
    def test_eviction_per_storage(self, storage):
        ...
        for i in range(15):
            storage.add_candle(
                create_test_candle(symbol="BTCUSDT", timestamp=1704067200000 + i * 60000)
            )
        # Fill ETH M1 storage
        for i in range(8):
            storage.add_candle(
                create_test_candle(symbol="ETHUSDT", timestamp=1704067200000 + i * 60000)
            )
...
class TestMemoryMonitoring:
    ...
    def test_storage_stats(self, storage):
        ...
        for i in range(5):
            storage.add_candle(
                create_test_candle(
                    symbol="BTCUSDT", timeframe=TimeFrame.M1, timestamp=1704067200000 + i * 60000
                )
            )
            storage.add_candle(
                create_test_candle(
                    symbol="ETHUSDT", timeframe=TimeFrame.M5, timestamp=1704067200000 + i * 300000
                )
            )
        stats = storage.get_stats()
...
class TestUtilityMethods:
    ...
    def test_get_candle_count(self, storage):
        ...
        for i in range(2):
            storage.add_candle(
                create_test_candle(
                    symbol="ETHUSDT", timeframe=TimeFrame.M1, timestamp=1704067200000 + i * 60000
                )
            )
        assert storage.get_candle_count() == 5
